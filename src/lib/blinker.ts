/**
 * @Author       : Humility
 * @Date         : 2022-12-05 17:37:22
 * @LastEditTime : 2022-12-05 17:37:26
 * @LastEditors  : Humility
 * @FilePath     : \miot-open-pc\src\lib\blinker.ts
 * @Description  :
 */
...
export class BlinkerDevice {
  ...
  getShareInfo() {
    axios
      .get(
        API.SHARE +
          `?deviceName=${this.config.deviceName}&key=${this.config.authKey}`
      )
      .then((resp) => {
        if (resp.data.message != 1000) {
          error(resp.data);
      ...
      this.mqttClient.on("message", (topic, message) => {
        if (topic == this.subTopic) {
          ...
          if (this.options.sourceCheck)
            if (
              this.sharedUserList.indexOf(fromDevice) < 0 &&
              fromDevice != this.config.uuid
            )
              return;
          this.processData(data, fromDevice);
        }
  ...
  startHeartbeat2cloud() {
    axios.get(
      API.HEARTBEAT +
        `?deviceName=${this.config.deviceName}&key=${this.config.authKey}&heartbeat=600`
    );
  ...
  sendMessage(message: string | Object, toDevice = this.targetDevice) {
    ...
    if (isJson(sendMessage)) {
      if (typeof this.messageDataCache[toDevice] == "undefined")
        this.messageDataCache[toDevice] = "";
      let ob =
        this.messageDataCache[toDevice] == ""
          ? {}
          : JSON.parse(this.messageDataCache[toDevice]);
      let ob2 = JSON.parse(sendMessage);
      this.messageDataCache[toDevice] = JSON.stringify(Object.assign(ob, ob2));
      if (typeof this.sendTimers[toDevice] != "undefined")
        clearTimeout(this.sendTimers[toDevice]);
      //检查设备是否是本地设备,是否已连接
      ...
      this.mqttClient.publish(
        this.pubTopic,
        formatMess2Device(this.config.deviceName, toDevice, sendMessage)
      );
    }
  }
  ...
  saveTsData(data: any) {
    ...
    if (
      ...
      currentData.date - this.storageCache[this.storageCache.length - 1].date >=
        5
    ) {
      this.storageCache.push(currentData);
    }
    if (
      this.storageCache[this.storageCache.length - 1].date -
        this.storageCache[0].date >=
        60 ||
  ...
  private sendTsData() {
    ...
    if (data.length > 10240) {
      error(
        "saveTsData:单次上传数据长度超过10Kb,请减少数据内容，或降低数据上传频率"
      );
      return;
    }
    tip("sendTsData");
    this.mqttClient.publish(
      this.pubTopic,
      formatMess2StorageTs(this.config.deviceName, "ts", data)
    );
    this.storageCache = [];
  }
  objectDataTimer;
  ...
  saveTextData(data: string) {
    ...
    this.textDataTimer = setTimeout(() => {
      ...
      this.mqttClient.publish(
        this.pubTopic,
        formatMess2StorageTt(this.config.deviceName, "tt", data)
      );
    }, 5000);
  }
  ...
  private addTimingTask(taskData) {
    ...
    this.timingTasks[taskData.task] = schedule.scheduleJob(config, () => {
      ...
      this.sendMessage(this.getTimingData());
      if (taskData.day == "0000000") this.disableTimingTask(taskData.task);
  ...
  setCountdownData(data) {
    if (data == "dlt") {
      ...
      JSON.stringify(data).indexOf(`{"run":1}`) > -1 ||
      JSON.stringify(data).indexOf(`{"run":0}`) > -1
    ) {
    ...
    this.countdownTimer2 = pauseable.setInterval(() => {
      ...
      if (
        this.tempData["countdown"]["rtim"] == this.tempData["countdown"]["ttim"]
      )
  ...
  sendRtData(key: string, func: Function, time = 1000) {
    if (typeof this.realtimeTasks[key] != "undefined")
      this.realtimeTasks[key].cancel();
    this.realtimeTasks[key] = schedule.scheduleJob(
      ...
      () => {
        let message = `{"${key}":{"val":${func()},"date":${Math.round(
